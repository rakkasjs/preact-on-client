    ...
    names.forEach((name, thing) => {
      ...
      switch (type) {
        ...
        case "Array":
          ...
          thing.forEach((v2, i) => {
            statements.push(`${name}[${i}]=${stringify(v2)}`);
          });
          break;
        case "Set":
          values.push(`new Set`);
          statements.push(
            `${name}.${Array.from(thing).map((v2) => `add(${stringify(v2)})`).join(".")}`
          );
          break;
        case "Map":
          values.push(`new Map`);
          statements.push(
            `${name}.${Array.from(thing).map(([k2, v2]) => `set(${stringify(k2)}, ${stringify(v2)})`).join(".")}`
          );
          break;
        default:
          values.push(
...
const { useContext, useEffect, useSyncExternalStore } = React14;
...
const { useContext: useContext4, useLayoutEffect } = React14;
  ...
  return useQuery(
    ...
    () => Promise.resolve(fn(closure, ctx)).then(async (result) => {
      if (process.env.RAKKAS_PRERENDER === "true") {
        ...
        const url = `/_data/${"w8yxoitiimh"}/` + moduleId + "/" + counter + closurePath + "/d.js";
...
async function renderApiRoute(ctx) {
  const apiRoutes = await import("./assets/virtual_rakkasjs_api-routes-82b5ea70.js");
...
async function renderPageRoute(ctx) {
  ...
  const routes = (await import("./assets/virtual_rakkasjs_server-page-routes-414bb579.js")).default;
  let {
    url: { pathname }
  } = ctx;
  ...
  let redirected;
  let status;
  let hold = process.env.RAKKAS_PRERENDER === "true" || false ? true : 0;
  function updateHeaders(props) {
    if (props.status) {
      status = typeof props.status === "function" ? props.status(status) : props.status;
    }
    if (hold !== true && true && props.throttleRenderStream !== void 0) {
...
function renderHead(prefetchOutput, renderMode, actionData = void 0, actionErrorIndex = -1, pageHooks = []) {
  let result = `<!DOCTYPE html><html><head>` + prefetchOutput + `<meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" />` + (renderMode === "hydrate" ? `<script>$RAKKAS_HYDRATE="hydrate"<\/script>` : "") + (actionData === void 0 && renderMode !== "server" ? "" : `<script>$RAKKAS_ACTION_DATA=${uneval(actionData)}<\/script>`);
...
var runServerSideServerHooks = {
  middleware: {
    beforeApiRoutes: async (ctx) => {
      ...
      const manifest = await import("./assets/virtual_rakkasjs_run-server-side_manifest-0c8e38ac.js");
